...
: 주어진 배열을 2개로 분할하고, 각각을 정렬

- 합병정렬과 다른 점
  
  - 합병정렬은 그냥 두 부분으로 나누는 반면, 퀵정렬은 분할할 때 기준 아이템(pivot item) 중심으로, 이보다 작은 것은 왼편, 큰 것은 오른편에 위치시킨
  
  - 각 부분 정렬이 끝난 후, 합병정렬은 '합병'이란 후처리 작업이 필요하나, 퀵정렬은 필요로하지 않음

- 시간 복잡도 : **O(nlogn)**, 최악은 O(n^2)
...
- 예시 - {69, 10, 30, 2, 16, 8, 31, 22} 퀵 정렬
  
  > 1. 원소의 개수가 8개이므로 4번째 자리에 있는 원소 2를 첫번째 피봇으로 선택하고 퀵 정렬 시작
  >    
  >    |    69    |    10    |    30    |     2     |    16    |    8    |    31    |    22   |
  >    |     L     |             |            |  pivot  |            |          |            |    R    |
  >    
  >    1. L이 오른쪽으로 이동하며 피복보다 크거나 같은 원소를 찾고, R은 왼쪽으로 이동하며 피봇보다 작은 원소 찾음
  >    
  >    2. L은 원소 `69`를 찾았지만, R은 피봇보다 작은 원소를 찾지 못한 채로 원소 `69`에서 L과 만남
  >    
  >    3. L과 R이 만났으므로, 원소 `69`를 피봇과 교환하여 피봇 원소 2의 위치 확정
  >       
  >         |    69    |    10    |    30    |     2     |    16    |    8    |    31    |    22   |
  >         |   L,R    |             |            |  pivot  |            |          |            |           |
  >       -> |   `2`  |    69    |    10    |    30    |    16    |    8    |    31    |    22   |
  > 
  > 2. 피봇 2의 왼쪽 부분 집합은 공집합이므로 퀵 정렬을 수행하지 않고, 오른쪽 부분 집합에 대해 퀵 정렬 수행
  >    
  >    1. 오른쪽 부분 집합의 원소가 7개이므로 가운데 있는 원소 16을 피봇으로 선택
  >    
  >    2. L이 찾은 30과 R이 찾은 8 서로 교환
  ...
  >          |            |            |     L     |            |  pivot  |    R    |            |           |
  > 
  > 3. 현재 위치에서 L과 R의 작업 반복
  > 
  > 4. L은 원소 69를 찾았지만, R은 피봇보다 작은 원소를 찾지 못한 채로 원소 69에서 L과 만남
  > 
  > 5. L과 R이 만났으므로, 원소 69를 피봇솨 교환해 피봇 원소 `16`의 위치 확정
  >       |   `2`    |    10    |     8    |   69    |    16    |   30    |    31    |    22   |
  >       |            |            |           |    L,R   |  pivot  |            |             |           |
  >    ->|    `2`   |    10    |     8    |   `16`    |    69    |   30    |    31    |    22   |
  >       |            |            |           |  pivot |            |            |             |           |
  > 
  > 6. 피봇 `16`의 왼쪽 부분 집합에서 원소 `10`을 피봇으로 선택하여 퀵 정렬 수행
  >    |    `2`     |    10    |     8    |   `16`    |    69    |   30    |    31    |    22   |
  >    |              |pivot,L|     R    |              |            |            |             |           |
  >    
  >    1. L의 원소 `10`과 R의 원소 `8`을 교환하는데, L의 원소가 피봇이므로 피봇 원소에 대한 자리 교환이 발생한 것이므로 교환한 자리를 피봇 원소 `10`의 위치로 확정한다
  >       |    `2`     |    10    |     8    |   `16`    |    69    |   30    |    31    |    22   |
  >       |              |pivot,L|     R    |              |            |            |             |           |
  >       |    `2`     |      8    |   `10`  |   `16`    |    69    |   30    |    31    |    22   |
  > 
  > 7. 피봇 `10`의 확정된 위치에서의 왼쪽 부분 집합은 원소가 1개이므로 퀵 정렬을 수행하지 않고, 오른쪽 부분 집합은 공집합이므로 역시 퀵 정렬을 수행하지 않는다.
  >    
  >    1. 이제 1단계 피봇이었던 원소 `16`에 대한 오른쪽 부분 집합에 대해 퀵정렬을 수행한다
  >    
  >    2. 오른쪽 부분 집합의 원소가 4개이므로 두번째 원소 `30`을 피봇으로 선택한다
  >    
  >    3. L이 찾은 `69`와 R이 찾은 `22`를 서로 교환한다
  >       
  >       |    `2`     |     8     |  `10`   |   `16`    |    69    |   30    |    31    |    22   |
  >       |              |            |            |              |    L      | pivot |             |    R     |
  >       |    `2`     |      8    |   `10`  |   `16`    |    22    |   30    |    31    |    69   |
  > 
  > 8. 현재 위치에서 L과 R의 작업을 반복한다. L은 오른쪽으로 이동하며 피봇보다 크거나 같은 원소인 `30`을 찾고, R은 왼쪽으로 이동하며 피봇보다 작은 원소를 찾다가 못 찾고 원소 `30`에서 L과 만난다
  >    |    `2`     |     8     |  `10`   |   `16`    |    22    |   30    |    31    |    69   |
  >    |              |            |            |              |         |pivot,L,R |          |            |
  >    |    `2`     |      8    |   `10`  |   `16`    |    22    |   `30`   |    31    |    69   |
  > 
  > 9. L과 R 이 만났으므로 피봇과 교환한다. 이 경우는 R의 원소가 피봇이므로 피봇에 대한 자리교환이 발생한 것이므로 교환한 자리를 피봇의 자리로 확정한다
  > 
  > 10. 피봇 `30`의 확정된 위치에서의 왼쪽 부분 집합의 원소가 1개이므로 퀵 정렬을 수행하지 않고, 오른쪽 부분 집합에 대해서 퀵 정렬을 수행한다
  >     
  >     1. 오른쪽 부분 집합의 원소 2개 중에서 원소 `31`을 피봇으로 선택한다.
  >     
  >     2. L은 오른쪽으로 이동하면서 원소 `31`을 찾고, R은 왼쪽으로 이동하면서 피봇보다 작은 원소를 찾다가 못 찾은 채로 원소 `31`에서 L과 만난다. L과 R이 만났으므로 피봇과 교환하는데 R의 원소가 피봇이므로 결국 제자리가 확정된다.
  >        |    `2`     |     8     |  `10`   |   `16`    |    22    |  `30`    |    31    |    69   |
  >        |              |            |            |              |           |            |pivot,L,R|          |
  >        |    `2`     |      8    |   `10`  |   `16`    |    22    |   `30`   |    `31`  |    69   |
  >     
  >     3. 피봇 `31`의 오른쪽 부분 집합의 원소가 1개이므로 퀵 정렬을 수행하지 않는다.
...
- 분할 정복 알고리즘 활용
  ...
  - 자료를 최소 단위의 문제까지 나눈 후에 차례대로 정렬해 최종 결과를 얻어냄
  
  - top-down 방식

- 시간 복잡도 : `O(n logn)`

- 예시 - `{69, 10, 30, 2, 16, 8, 31, 22}` 병합 정렬
  
  > 1. 전체 자료 집합에 대해 최소 크기의 부분집합이 될 때까지 분할 작업
  > 
  > 2. 2개의 부분집합을 정렬하면서 하나의 집합으로 병합\
  > 
  > ```pseudocode
  > # 분할 과정
  > merge_sort(LIST m)
