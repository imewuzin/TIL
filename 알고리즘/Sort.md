...
- 예시 - {69, 10, 30, 2, 16, 8, 31, 22} 퀵 정렬
  ...
  > 
  > 4. 현재 위치에서 L과 R의 작업을 반복한다. L은 오른쪽으로 이동하며 피봇보다 크거나 같은 원소인 `30`을 찾고, R은 왼쪽으로 이동하며 피봇보다 작은 원소를 찾다가 못 찾고 원소 `30`에서 L과 만난다
  >    |    `2`     |     8     |  `10`   |   `16`    |    22    |   30    |    31    |    69   |
  >    |              |            |            |              |         |pivot,L,R |          |            |
  >    |    `2`     |      8    |   `10`  |   `16`    |    22    |   `30`   |    31    |    69   |
  > 
  > 6. L과 R 이 만났으므로 피봇과 교환한다. 이 경우는 R의 원소가 피봇이므로 피봇에 대한 자리교환이 발생한 것이므로 교환한 자리를 피봇의 자리로 확정한다
  > 
  > 7. 피봇 `30`의 확정된 위치에서의 왼쪽 부분 집합의 원소가 1개이므로 퀵 정렬을 수행하지 않고, 오른쪽 부분 집합에 대해서 퀵 정렬을 수행한다
  >    
